...
from polymarket_glm.strategy.estimator import EstimateResult, MarketInfo, ProbabilityEstimator
...
# ── Weighted Ensemble (multi-estimator combination) ──────────────
...
class WeightedEnsembleEstimator:
    ...
    result = await ensemble.estimate(market_info)
    """
    ...
    async def estimate(self, market: MarketInfo) -> EstimateResult:
        ...
        for we in self._estimators:
            r = await we.estimator.estimate(market)
            results.append(r)
